...
void EarthVisualization::generateMesh()
{
	...
	while (m_nTrCount < 2000)
	{
		...
		core::vector3df* ptrVertPos = 0;
		if (i->getMarker() == triangle)
		{
			ptrVertPos = m_Sphere.getVertex(vertexId);
		}
		if (!ptrVertPos || isPointVisibleAtLevel(ptrVertPos->getDistanceFrom(
				startPt), level))
		{
			//if (!addingStarted)
			//{
		...
		else
		{
			...
			if (addingStarted)
			{
				...
				log->debug("New level %d at tr=%s", level, str.c_str());
				...
				do
				{
					...
					if (!newIterator->isUsed(triangle))
					{
						if (!markerSet)
						{
							newIterator->setMarker(triangle);
						}
						newIterator->setUsed(triangle);
						if (newIterator->accept(triangle))
						{
							markerSet = true;
						}
					}
