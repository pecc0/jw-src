...
class BFSIterator: virtual public IReferenceCounted
{
	...
	u32 m_trMarker;
public:
	BFSIterator(JWSphere* sphere, int level, u32 marker);
	...
	/**
	 * Current BFS iteration - starts from 0 and will be incremented at each new "wave" of the BFS
	 */
	int getIteration();

	/**
	 * Marker to show where w increase the iteration
	 */
	u32 getMarker();
